...
public class ExportOrderJDBCServiceImpl implements ExportServiceImpl {
    private String jdbcURL = "jdbc:mysql//localhost:3306/inventor_manager";
    ...
    private static final String INSERT_USER_SQL = "INSERT INTO export_order " + "(name,createBy) VALUES"
            + "(?,?);";
//    private static final String SELECT_USER_BY_IDEXPORTORDER = "SELECT idExportOrder,name,exportDate,createBy,modifyBy form export_order WHERE idExportOrder = ?; ";
    private static final String SELECT_ALL_EXPORTORDER = "SELECT idExportOrder,name,exportDate,createBy FORM export_order WHERE isDelete = 0;";
    private static final String DELETE_USER_SQL = "DELETE FROM export_order WHERE idExportOrder = ?;";
    private static final String UPDATE_USER_SQL = "UPDATE export_order SET name=?,modifyBy=?;";
    ...
    @Override
    public void update(String name, ExportOrder exportOrder){
        boolean rowUpdate;
        try(Connection connection = getConnection();
